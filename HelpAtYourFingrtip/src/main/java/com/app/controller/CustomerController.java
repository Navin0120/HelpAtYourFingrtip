...
public class CustomerController {
	...
	public ResponseEntity<?> processCustomerUpdate(@RequestBody Customer customer,@PathVariable int custId){
		return ResponseEntity.ok().body(cusService.updateCustomerDetails(customer, custId));
	}
	
	@PatchMapping("/updatePassword/{custId}")
	public ResponseEntity<?> processCustomerPassword1(@RequestBody String password,@PathVariable int custId){
		return ResponseEntity.ok().body(cusService.updatePassword(password, custId));
	}
